...
var Renderer = {
  ...
  parseEntities: function(text, entities) {
    ...
    for (i = 0, len = orderedEntities.length; i < len; ++i) {
      ...
      Renderer.addText(
        elements,
        Transforms.transformEntities(text.substring(totalInc, indices[0]))
      );
      ...
      if (entity.type === 'mention') {
        ...
        Renderer.addText(
          elements,
          Transforms.transformEntities(text.substring(indices[0], indices[1]))
        );
    ...
    Renderer.addText(
      elements,
      Transforms.transformEntities(text.substring(totalInc, text.length))
    );
  ...
  addText: function (parent, content) {
    textContents = content.split(/\r?\n/);
    parent.appendChild(Renderer.makeText(textContents[0]));
    for(var j = 1; j < textContents.length; j++) {
      parent.appendChild(document.createElement("br"));
      parent.appendChild(Renderer.makeText(textContents[j]));
    }
  },
  ...
  imagesFromEntities: function(entities)
  {
    ...
    {
      entity = entities[i];
      var img = Renderer.makeImage( entity.media_url_https );
      img.setAttribute( 'style', 'max-width: 100%; max-height: ' 
        + OptionsBackend.get('photo_thumbnail_max_height') + 'px;' );
      AnyClick.anyClick(img, Renderer.openImageSource(entity));
      elements.appendChild( img );

      if(entity.video_info) {
        var overlay = Renderer.makeImage( "img/play.png" );
        overlay.classList.add("play");
        img.classList.add("video_preview");
        elements.appendChild( overlay );
      }
    }
    return elements;
  },

  openImageSource: function(entity) {
    return function() {
      if(entity.video_info) {
        openTab(entity.video_info.variants[0].url);
      } else {
        openTab(entity.media_url_https + ":large");
      }
    }
  },
  ...
  renderTweet: function (tweet, useColors, nameAttribute, topLevel) {
    var user = tweet.user;
    var text = tweet.text;
    var tweetId = tweet.id;
    var entities = tweet.entities;
    var media_entities = tweet.extended_entities || tweet.entities;
    if(tweet.retweeted_status) {
      user = tweet.retweeted_status.user;
      text = tweet.retweeted_status.text;
      tweetId = tweet.retweeted_status.id;
      entities = tweet.retweeted_status.entities;
      media_entities = tweet.retweeted_status.extended_entities || tweet.retweeted_status.entities;
    }
    var content;
    if (entities) {
    ...
    if(OptionsBackend.get('render_emoji')){
    ...
    if(tweet.quoted_status_id) {
      //tweet.quoted_status.quote = true;
      var linkDst = TwitterLib.URLS.BASE + '/statuses/' + tweet.quoted_status_id_str;
      quoting = document.createElement("a");
      quoting.setAttribute("href", "#");
      quoting.appendChild(Renderer.makeText(chrome.i18n.getMessage("quoting") 
        + ((tweet.quoted_status) ? ( ' ' + tweet.quoted_status.user.screen_name ) : ' (' + chrome.i18n.getMessage("someone") + ')')));
      var replyAction;
      if(this.isNotification()) {
        replyAction = function() {
          openTab(linkDst);
        ...
        replyAction = function() {
          Renderer.toggleQuote(tweet, tweetSpace);
        };
      }
      AnyClick.anyClick(quoting, replyAction);
      if(typeof tweet.quoteVisible == "undefined") tweet.quoteVisible = topLevel && OptionsBackend.get('auto_expand_quotes');
      Renderer.expandQuote(tweet, tweetSpace, true);
    }
    ...
    if(this.isComplete() && tweet.quote) {
      overlayStyle = 'background-color: ' + OptionsBackend.get('quotes_tweets_color') + ';';
    }
    ...
    var avatar_container = Renderer.makeDiv({'class':"avatar_container"});
    first_container.appendChild(avatar_container);

    // now in container -> overlay -> first_container -> avatar_container
    if(tweet.retweeted_status) {
      ...
      avatar_container.appendChild(img1);
      avatar_container.appendChild(img2);
    ...
    // exiting avatar_container, now container -> overlay -> first_container

    var main_container = Renderer.makeDiv({'class':"main_container"});
    first_container.appendChild(main_container);

    // now in container -> overlay -> first_container -> main_container
    ...
    main_container.appendChild(userName);

    if(user.verified) {
      main_container.appendChild(Renderer.makeText(" "));
      main_container.appendChild(Renderer.makeElem("img", {'class':'verified', src:'img/verified.png', alt:'verified'}));
    }
    if(user['protected']) {
      main_container.appendChild(Renderer.makeText(" "));
      main_container.appendChild(Renderer.makeElem("img", {'class':'protected', src:'img/lock.png', alt:'protected'}));
    }

    var textDiv = Renderer.makeDiv({'class':'text'});
    main_container.appendChild(textDiv);
    ...
    // exiting textDiv, now container -> overlay -> first_container -> main_container
    // exiting textDiv, now container -> overlay -> first_container
    // exiting first_container, now container -> overlay

    if(OptionsBackend.get('show_photo_thumbnails')) {
      var imgDiv = Renderer.makeDiv({'class':'image'});
      imgDiv.appendChild(this.imagesFromEntities(media_entities));
      main_container.appendChild(imgDiv);
    }
  ...
  expandQuote: function(tweet, target, showIfVisible) {
    if(showIfVisible && !tweet.quoteVisible) {
      return;
    }

    $("#loading").show();
    tweetManager.getQuotedTweet(function(success, data, status) {
      if(success) {
        tweet.quoted_status.quote = true;
        tweet.quoteVisible = true;
        var renderedTweet = Renderer.renderTweet(data, false, OptionsBackend.get('name_attribute'));
        target.appendChild(renderedTweet);
        var separator = Renderer.makeDiv({'class': 'reply_separator'}, "\u2193");
        renderedTweet.insertBefore(separator, renderedTweet.childNodes[0]);
        separator.onclick = function() {
          Renderer.toggleQuote(tweet, target);
        };
        if(!showIfVisible) {
          $(renderedTweet).show('blind', { direction: "vertical" });
        }
  ...
  toggleQuote: function(tweet, target) {
    if(tweet.quoteVisible) {
      tweet.quoteVisible = false;
      var tweetSpace = $("div.tweet[tweetid='" + tweet.quoted_status_id + "']", target).parents('.tweet_space');
      tweetSpace.first().hide('blind', { direction: "vertical" }, 'normal', function() {
        $(this).remove();
      });
      return;
    ...
    Renderer.expandQuote(tweet, target);
  },
